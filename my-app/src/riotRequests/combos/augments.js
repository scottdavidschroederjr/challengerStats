...
async function getMatches (PUUID, setNumber) {
    matchUsers.belongsTo(generalMatchData, {foreignKey: 'matchID'})
    generalMatchData.hasMany(matchUsers, {foreignKey: 'matchID'})
    ...
    console.log(matchList[0]['dataValues'])
    console.log(matchList[0]['dataValues']['generalMatchDatum'])
    return matchData
    } catch(error) {
        console.log(error)
    }
